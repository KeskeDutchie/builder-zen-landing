...
export default function BuildEditor() {
  ...
  const [checkedMantras, setCheckedMantras] = useState([]);
  ...
  const updateStat = (index, newValue) => {
  ...
  const getStatMaxValue = (currentValue) => {
    return Math.min(100, currentValue + remainingStatPoints);
  };

  const getAttunementMaxValue = (currentValue) => {
    return Math.min(100, currentValue + remainingStatPoints);
  };

  const getWeaponMaxValue = (currentValue) => {
    return Math.min(100, currentValue + remainingStatPoints);
  };

  const getAttunementRecommendations = (attunement) => {
    switch (attunement) {
      case "Flamecharm":
  ...
  const updateAttunement = (attunement, value) => {
    setBuild((prev) => ({
      ...prev,
      attunements: {
        ...prev.attunements,
  ...
  const updateWeapon = (weapon, value) => {
    setBuild((prev) => ({
      ...prev,
      weapons: {
        ...prev.weapons,
